...
**Non-negotiables:** **mobile-first** (~95% of visitors are on phones via WhatsApp forwards) and **SEO-strong** (the community grows through search + shares). The **dynamic tier is Supabase-backed** (free tier, auth + Row-Level Security): **community board**, **Events**, **Newcomer's Guide**, **Announcements**, **FAQs**, **Guidelines**, **Safety Disclaimers**, and **Businesses** (all admin-managed in-site, server-rendered with ISR). The **static info pages** (home, groups, contact) stay static and free to host. Keep the split clean — a *static* info page must never depend on the database.
...
- **Events**, **Newcomer's Guide**, **Announcements**, **FAQs**, **Guidelines**, **Safety Disclaimers**, and **Businesses** are admin-managed in-site via `app/admin-actions.ts` (gated by `is_admin`), rendered by their respective Admin components.
...
app/            static info pages + sitemap.ts, robots.ts, opengraph-image.tsx, not-found.tsx, error.tsx, goodbye/
  ...
  announcements/, faq/, guidelines/, safety/, businesses/  ← DB-backed, admin CRUD (ISR)
  ...
  admin-actions.ts  admin/moderator server actions (events, newcomers, announcements, faqs, guidelines, safety, businesses, reports, team)
components/      Header, Footer, StickyJoinBar, JoinButton, GroupCard, BusinessCard, QrCode, LogoMark,
                ...
                ContributeBanner, SignedOutOnly, EventAdmin, NewcomerAdmin, AnnouncementAdmin, FaqAdmin,
                GuidelineAdmin, SafetyDisclaimerAdmin, BusinessAdmin
  community/    PostCard, PostForm, VoteButtons, ReportButton, AnswerList, AnswerForm, AuthButtons
data/           groups, site, + seed references (events, newcomers, announcements, faqs, guidelines, safety, businesses)
...
- **Admin-managed content** (Announcements, FAQs, Guidelines, Safety Disclaimers, Businesses, Events, Newcomer's Guide): Sign in as an admin and use the add/edit/delete controls on the respective pages (`/announcements`, `/faq`, `/guidelines`, `/safety`, `/businesses`, `/events`, `/newcomers`). Each page is server-rendered with ISR (still SEO-indexable). The old `data/*.ts` files remain only as migration seed/reference; do not edit them. `/faq` also emits FAQPage JSON-LD for rich results.
...
- **Safety Disclaimers:** only the `/safety` page's bulleted disclaimer list is DB-backed. The footer disclaimer (every page, via `Footer.tsx`) and the community disclaimer (post form + modal gate, via `data/safety.ts` hardcoded constants) are unchanged.
...
## Roadmap
The v2 community board (Q&A + housing + marketplace) is **built** — Supabase-backed, additive to the site (see `supabase/SETUP.md` to provision it). v4 adds **admin moderation** (reports queue at `/admin`, email alerts, team management at `/admin/team`) and **admin-managed content** (Announcements, FAQs, Guidelines, Safety Disclaimers, Businesses all migrated from `data/*.ts` to Supabase with in-site CRUD). Future ideas: in-app notifications, saved searches, richer moderation tooling (report comments, etc.).
